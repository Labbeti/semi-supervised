...
def run(cfg: DictConfig) -> None:
    ...
    if cfg.mt.activation == "sigmoid":
        ...
        raise ValueError(
            f"activation {cfg.mt.activation} is not available. available: [sigmoid | sofmax]"
        )
    ...
    def train(iteration, S, U, start_time):
        ...
        ccost = consistency_cost(
            activation_fn(student_logits), activation_fn(teacher_logits)
        )
        ...
        with torch.no_grad():
            ...
            fscore_ss = metrics.sup.fscore_s(activation_fn(student_s_logits), y_s).mean(
                size=100
            )
            fscore_su = metrics.unsup.fscore_s(
                activation_fn(student_u_logits), y_u
            ).mean(size=100)
            ...
            fscore_ts = metrics.sup.fscore_t(activation_fn(teacher_s_logits), y_s).mean(
                size=100
            )
            fscore_tu = metrics.unsup.fscore_t(
                activation_fn(teacher_u_logits), y_u
            ).mean(size=100)
    ...
    def val(iteration):
        ...
        with torch.no_grad():
            for i, (X, y) in enumerate(val_loader):
                ...
                fscore_s = metrics.sup.fscore_s(activation_fn(student_logits), y).mean(
                    size=100
                )
                fscore_t = metrics.sup.fscore_t(activation_fn(teacher_logits), y).mean(
                    size=100
                )
                mAP_s = metrics.sup.map_s(
                    activation_fn(student_logits).cpu().reshape(-1), y.cpu().reshape(-1)
                ).mean(size=100)
                mAP_t = metrics.sup.map_s(
                    activation_fn(teacher_logits).cpu().reshape(-1), y.cpu().reshape(-1)
                ).mean(size=100)
        ...
        tensorboard.add_scalar(
            "hyperparameters/learning_rate", get_lr(optimizer), iteration
        )
        tensorboard.add_scalar(
            "hyperparameters/lambda_cost_max", lambda_cost(), iteration
        )

        tensorboard.add_scalar(
            "max/student_acc", maximum_tracker("student_acc", acc_s), iteration
        )
        tensorboard.add_scalar(
            "max/teacher_acc", maximum_tracker("teacher_acc", acc_t), iteration
        )
        tensorboard.add_scalar(
            "max/student_f1", maximum_tracker("student_f1", fscore_s), iteration
        )
        tensorboard.add_scalar(
            "max/teacher_f1", maximum_tracker("teacher_f1", fscore_t), iteration
        )
        tensorboard.add_scalar(
            "max/student_map", maximum_tracker("student_map", mAP_s), iteration
        )
        tensorboard.add_scalar(
            "max/teacher_map", maximum_tracker("teacher_map", mAP_t), iteration
        )
