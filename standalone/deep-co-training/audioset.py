...
from SSL.loss.losses import (
    DCTSupWithLogitsLoss,
    DCTDiffWithLogitsLoss,
    DCTCotWithLogitsLoss,
    Activation,
    ValidLoss,
)
...
def run(cfg: DictConfig) -> DictConfig:
    ...
    losses = DotDict(
        {
            "sup": DCTSupWithLogitsLoss(
                reduction="mean", sub_loss=ValidLoss.BINARY_CROSS_ENTROPY
            ),
    ...
    def train(iteration, S1, S2, U, start_time):
        ...
        l_diff = losses.diff(
            (logits_s1, adv_logits_s1),
            (logits_s2, adv_logits_s2),
            (logits_u1, adv_logits_u1),
            (logits_u2, adv_logits_u2),
        )
    ...
    def val(iteration):
        ...
        with torch.set_grad_enabled(False):
            for i, (X, y) in enumerate(val_loader):
                ...
                mAP_1 = m_(
                    metrics.sup.map_1(
                        S(logits_1).cpu().reshape(-1), y.cpu().reshape(-1)
                    )
                )
                mAP_2 = m_(
                    metrics.sup.map_2(
                        S(logits_2).cpu().reshape(-1), y.cpu().reshape(-1)
                    )
                )
                avg_sup = m_(metrics.avg.sup(l_sup.item()))
        ...
        tensorboard.add_scalar(
            "max/fscore_1", maximum_tracker("fscore_1", fscore_1), iteration
        )
        tensorboard.add_scalar(
            "max/fscore_2", maximum_tracker("fscore_2", fscore_2), iteration
        )
        ...
        tensorboard.add_scalar(
            "detail_hyperparameters/lambda_cot", lambda_cot(), iteration
        )
        tensorboard.add_scalar(
            "detail_hyperparameters/lambda_diff", lambda_diff(), iteration
        )
        tensorboard.add_scalar(
            "detail_hyperparameters/learning_rate", get_lr(optimizer), iteration
        )
